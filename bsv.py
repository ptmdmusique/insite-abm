...
class BSV:
    ...
    def __init__(self, agents, power_attribute, preference_attribute,
        ...
        for a in self.agent_list:
            ...
            # primary
            self.net.nodes[node]["power"] = power(a)
            # duplicate
            self.orignet.nodes[node]["power"] = power(a)
            # primary
            self.net.nodes[node]["preference"] = preference(a)
            # duplicate
            self.orignet.nodes[node]['preference'] = preference(a)
            # primary
            self.net.nodes[node]["maybe_mates"] = []
            # duplicate
            self.orignet.nodes[node]["maybe_mates"] = []
            # primary
            self.net.nodes[node]['g_dis'] = False
            # duplicate
            self.orignet.nodes[node]["g_dis"] = False
    ...
    def make_subnets(self, newname, node1, node2, newpref):
        ...
        for each in group_list:
            ...
            diff = self.orignet.nodes[each]['preference'] \
                + ((newpref - self.orignet.nodes[each]['preference'])
                   * self.comp)
            self.subnets[newname].add_node(each)
            self.subnets[newname].nodes[each]['preference'] = diff
            self.orignet.nodes[each]['preference'] = diff
            # update reference dictionary with
            # new afFinity value based on alliances
            self.subnets[newname].nodes[each]['power'] = \
                self.orignet.nodes[each]['power']
            self.subnets[newname].nodes[each]['maybe_mates'] = \
                self.orignet.nodes[each]['maybe_mates']
            self.subnets[newname].nodes[each]['g_dis'] = \
                self.orignet.nodes[each]['g_dis']
                            ...
                            if maybe_mate[1] not in allied:
                                ...
                                for i in network.nodes[maybe_mate[1]]["maybe_mates"]:
                                    ...
                                    if i[1] == node[0]:
                                        ...
                                        if network.nodes[maybe_mate[1]]['maybe_mates'].\
                                                index(i) <= poss:
                                            # make alliance
                                            network.add_edge(
                                                node[0], maybe_mate[1])
    ...
    def new_node(self, network):
        ...
        for one, two in network.edges():
            ...
            prefA = network.nodes[one]['preference']
            prefB = network.nodes[two]['preference']
            powA = network.nodes[one]['power']
            powB = network.nodes[two]['power']
            # calculate new preference
        ...
        for i in new_nodes:
            ...
            network.nodes[newname]['power'] = new_pows[idx]
            # add new affinity attributes
            network.nodes[newname]['preference'] = new_prefs[idx]
            # create empy possible mates key
            network.nodes[newname]['maybe_mates'] = []
            network.nodes[newname]['g_dis'] = 'False'
    ...
    def check_alliances(self, subs, nets):
        ...
        for item in dis:
            ...
            nets.nodes[item[0]]['power'] = \
                self.orignet.nodes[item[0]]['power']
            nets.nodes[item[0]]['preference'] = \
                self.orignet.nodes[item[0]]['preference']
            nets.nodes[item[0]]['maybe_mates'] = []
            nets.nodes[item[0]]['g_dis'] = "True"
